...
print('\nMirkli uzgaidīt, tiek gatavots grafiks...\n\n')
...
fDates = Source(xw.books.active, xw.sheets['List'], df(xw.books.active.sheets['List'].range('ListTable[[#All]]').value))   #xw.books.active
...
[c.clean_up_df() for c in [fDates, fNames, fCountr, cFields]]  # izsauc metodi, kas izmet tukšās kolonnas, nomet pirmo rindu, pārsauc galveni
...
targ_wb.api.ConvertComments() # converts from notes to threaded comments (Office 2010+)
targ_wb.save()